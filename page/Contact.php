<div class="contact">
    <div class="nes-container with-title">
        <p class="title">Mes coordonnées</p>
        <span>Email: <a href="mailto:jordan@pandatown.fr">jordan@pandatown.fr</a></span>
        <span>Number: <span>+33 7 69 39 23 72</span></span>
    </div>
    <div class="nes-container with-title">
        <p class="title">Formulaire de contact</p>
        <div class="nes-field is-inline">
            <label for="name">Name</label>:
            <input type="text" id="name" class="nes-input">
        </div>
        <div class="nes-field is-inline">
            <label for="email">email</label>:
            <input type="text" id="email" class="nes-input">
        </div>
        <div class="nes-field is-inline">
            <label for="phone">phone</label>:
            <input type="text" id="phone" class="nes-input">
        </div>
        <div class="nes-field">
            <label for="description">Détail</label>
            <textarea id="description" class="nes-textarea" placeholder="Merci de détailler le plus possible votre besoin afin de faciliter nos futurs échanges."></textarea>
        </div>
        <button id="sendcontact" type="button" class="nes-btn is-primary">Envoyé</button>
    </div>
</div>